...
{ 
public: 
    SampleParameter() : rclcpp::Node("SampleParameter")
    { 
        declare_parameter<int>("simple_int_param", 28);
        declare_parameter<std::string>("simple_string_param", "Ros_parameter_name");

        param_callback_handle_ = add_on_set_parameters_callback(std::bind(&SampleParameter::paramChangeCallback, this, _1)); 
    }
...
int main(int argc, char* argv[])
{
    ...
    auto node = std::make_shared<SampleParameter>(); 
    rclcpp::spin(node); 
    rclcpp::shutdown(); 
    return 0; 
}